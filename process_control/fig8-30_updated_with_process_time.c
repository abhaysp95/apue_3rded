...
static void pr_times(const char* whoami, const clock_t real, const struct tms* rstart, const struct tms* rend);
...
struct timeval end;  // end time (time resolution)
...
#error NZERO undefined
	...
	while (1) {
		...
		count++;
		if (checktime(whoami)) {
			if ((rend = times(&tmsend)) == -1) {
				err_sys("times() [end] error");
			}
			pr_times(whoami, rend - rstart, &tmsstart, &tmsend);
			exit(0);
		}
	}
...
static void pr_times(const char* whoami, const clock_t real, const struct tms* tmsstart, const struct tms* tmsend) {
...
#error _SC_CLK_TCK not defined
	...
	// printf("time for \"%s\"\n", whoami);
	printf("\t[%s] real: %7.2f\n", whoami, real / (double) clktck);
	printf("\t[%s] user: %7.2f\n", whoami, (tmsend->tms_utime - tmsstart->tms_utime) / (double) clktck);
	printf("\t[%s] system: %7.2f\n", whoami, (tmsend->tms_stime - tmsstart->tms_stime) / (double) clktck);
	printf("\t[%s] child user: %7.2f\n", whoami, (tmsend->tms_cutime - tmsstart->tms_cutime) / (double) clktck);
	printf("\t[%s] child system: %7.2f\n", whoami, (tmsend->tms_cstime - tmsstart->tms_cstime) / (double) clktck);
}

/**
 * NOTE: how can I measure cpu usage for these 2 process (parent & child)
 * seperately and same for process times for these two ? (process times implemented, obviously it would ~= 10sec)
 */
